...
class Quench:
    ...
    def __init__(self, nvim):
        ...
        self.web_server = WebServer(
            ...
            auto_select_port=self._cached_web_server_auto_select_port,
        )
    ...
    def _get_or_select_kernel_sync(self, bnum):
        ...
        if bnum in self.kernel_manager.buffer_to_kernel_map:
            ...
            if kernel_id in self.kernel_manager.sessions:
                ...
                return {"value": kernel_id, "is_running": True, "kernel_choice": session.kernel_name}
    ...
    async def _run_cell_async(self, current_bnum, cell_code, kernel_choice):
        ...
        if kernel_choice["is_running"]:
            ...
            session = await self.kernel_manager.get_or_create_session(
                current_bnum, self.relay_queue, buffer_name, kernel_choice_value
            )
    ...
    async def _handle_message_for_nvim(self, kernel_id: str, message: dict):
        ...
        try:
            ...
            self._logger.debug(
                f"Processing message type: {msg_type}, content keys: {list(content.keys()) if content else 'None'}, parent_msg_id: {parent_msg_id}"
            )
            ...
            if msg_type == "stream":
                ...
                def notify_kernel_death():
                    notify_user(
                        self.nvim,
                        f"Kernel {kernel_id[:8]} died. It will auto-restart on next execution.",
                        level="error",
                    )
    ...
    async def _interrupt_kernel_async(self, current_bnum):
        ...
        try:
            ...
            def notify_error():
                notify_user(self.nvim, f"Failed to interrupt kernel: {error_msg}", level="error")
    ...
    async def _reset_kernel_async(self, current_bnum):
        ...
        try:
            ...
            def notify_error():
                notify_user(self.nvim, f"Failed to reset kernel: {error_msg}", level="error")
    ...
    def open_command(self):
        ...
        if not self.web_server_started or not self.web_server:
            notify_user(
                self.nvim, "Web server is not running. Run :QuenchStartKernel or :QuenchRunCell first.", level="error"
            )
            return
    ...
    def select_kernel_command(self):
        ...
        return self.async_executor.execute_sync(
            self._select_kernel_async(current_bnum, selected_choice), "kernel selection"
        )
